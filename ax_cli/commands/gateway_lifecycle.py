"""ax gateway lifecycle — managed agent start/stop/attach commands.

Extracted from ``commands/gateway.py`` per issue #28 Phase 1. Owns the
desired-state toggle, the manual-attach marker, the external runtime
announcement, and the attached-session Claude Code launcher. The matching
``agents start | stop | mark-attached | attach`` CLI commands live here
too and are registered onto ``agents_app`` from ``commands/gateway.py``.
"""
...
# Track attached Claude Code child processes so the gateway parent can
# reap them on shutdown / reattach. Module-level so a fresh attach call
# can prune any stale entries before adding a new one.
...
def _prepare_attached_agent_payload(name: str) -> dict:
    ...
    _set_state = getattr(_gateway_cmd, "_set_managed_agent_desired_state", _set_managed_agent_desired_state)
    _attach_cmd = getattr(_gateway_cmd, "_attach_command_for_payload", _attach_command_for_payload)
    ...
    payload["attach_command"] = _attach_cmd(payload)
    _set_state(name, "running")
    return payload
...
# ---------------------------------------------------------------------------
# agents sub-app commands (start / stop / mark-attached / attach).
# Registered against ``agents_app`` in ``commands/gateway.py``.
# ---------------------------------------------------------------------------
...
def mark_attached_agent(
    ...
    _mark = getattr(_gateway_cmd, "_mark_attached_agent_session", _mark_attached_agent_session)
    try:
        payload = _mark(name, note=note)
...
def attach_agent(
    ...
    _prepare = getattr(_gateway_cmd, "_prepare_attached_agent_payload", _prepare_attached_agent_payload)
    try:
        payload = _prepare(name)
    except LookupError:
        err_console.print(f"[red]Managed agent not found:[/red] {name}")
        raise typer.Exit(1)
