"""ax gateway session — local pass-through session protocol (fingerprint, challenge, message)."""
...
def _find_local_origin_collision(registry: dict, *, fingerprint: dict, requested_name: str) -> dict | None:
    ...
    _origin_signature = getattr(_gateway_cmd, "_local_origin_signature", _local_origin_signature)
    origin_signature = _origin_signature(fingerprint)
    ...
    for candidate in registry.get("agents") or []:
        ...
        if not isinstance(candidate_fingerprint, dict):
            continue
        if _origin_signature(candidate_fingerprint) == origin_signature:
            return candidate
    return None
...
def _connect_local_pass_through_agent(
    *,
    ...
    _verify = getattr(_gateway_cmd, "_local_fingerprint_verification", _local_fingerprint_verification)
    _find_collision = getattr(_gateway_cmd, "_find_local_origin_collision", _find_local_origin_collision)
    _register = getattr(_gateway_cmd, "_register_managed_agent")
    _hydrate = getattr(_gateway_cmd, "_hydrate_entry_space_from_database")
    _trust_signature = getattr(_gateway_cmd, "_local_trust_signature", _local_trust_signature)
    _with_refs = getattr(_gateway_cmd, "_with_registry_refs")
    _ref_for_agent = getattr(_gateway_cmd, "_registry_ref_for_agent")
    ...
    verification = _verify(fingerprint)
    ...
    entry["local_trust_signature"] = _trust_signature(normalized_name, fingerprint)
    ...
    payload = {
        ...
        "agent": _with_refs(registry, annotate_runtime_health(entry, registry=registry)),
        "approval_id": entry.get("approval_id"),
        "registry_ref": _ref_for_agent(registry, entry),
        "fingerprint": fingerprint,
...
def _ensure_session_challenge(
    ...
    *,
    ...
    _find_record = getattr(_gateway_cmd, "_find_local_session_record", _find_local_session_record)
    _generate = getattr(_gateway_cmd, "_generate_session_challenge_code", _generate_session_challenge_code)

    record = _find_record(registry, session_id)
    if record is None:
...
def _send_local_session_message(*, session_token: str, body: dict) -> dict:
    ...
    _challenge_enabled = getattr(_gateway_cmd, "_gateway_session_challenge_enabled", _gateway_session_challenge_enabled)
    _ensure_challenge = getattr(_gateway_cmd, "_ensure_session_challenge", _ensure_session_challenge)
    ...
    if _challenge_enabled():
        next_session_proof = _ensure_challenge(
            registry,
...
def _proxy_local_session_call(*, session_token: str, body: dict) -> dict:
    ...
    _load_client = getattr(_gateway_cmd, "_load_managed_agent_client")
    _proxy_methods = getattr(_gateway_cmd, "_LOCAL_PROXY_METHODS", _LOCAL_PROXY_METHODS)

    method = str(body.get("method") or "").strip()
    if method not in _proxy_methods:
        raise ValueError(f"method not on Gateway proxy allowlist: {method!r}")
    spec = _proxy_methods[method]
