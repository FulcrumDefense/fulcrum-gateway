"""ax gateway local — `local` sub-app commands for local pass-through agents."""
...
def _resolve_local_gateway_identity(
    *,
    ...
    _config_from_workdir = getattr(
        _gateway_cmd, "_gateway_local_config_from_workdir", _gateway_local_config_from_workdir
    )

    workdir_cfg = _config_from_workdir(workdir)
...
def _request_local_connect(
    *,
    ...
    _resolve_identity = getattr(_gateway_cmd, "_resolve_local_gateway_identity", _resolve_local_gateway_identity)
    _fingerprint = getattr(_gateway_cmd, "_local_process_fingerprint")
    _guidance = getattr(_gateway_cmd, "_local_route_failure_guidance", _local_route_failure_guidance)

    resolved_workdir = str(Path(workdir or Path.cwd()).expanduser().resolve())
    agent_name, registry_ref = _resolve_identity(
    ...
    fingerprint = _fingerprint(agent_name=display_name, cwd=resolved_workdir)
...
def _resolve_local_gateway_session(
    *,
    ...
    _connect = getattr(_gateway_cmd, "_request_local_connect", _request_local_connect)
    _guidance = getattr(_gateway_cmd, "_approval_required_guidance", _approval_required_guidance)
...
def _check_local_pending_replies(
    *,
    ...
    _poll = getattr(_gateway_cmd, "_poll_local_inbox_over_http", _poll_local_inbox_over_http)
...
def local_init(
    ...
    _ensure = getattr(_gateway_cmd, "_ensure_workdir", _ensure_workdir)
    _config_text = getattr(_gateway_cmd, "_gateway_local_config_text", _gateway_local_config_text)
    _connect = getattr(_gateway_cmd, "_request_local_connect", _request_local_connect)

    raw_workdir = workdir or str(Path.cwd())
    root = Path(raw_workdir).expanduser().resolve()
    _ensure(root, create=create_workdir, raw_input=raw_workdir)
    ax_dir = root / ".ax"
    ...
    config_path.write_text(
        _config_text(agent_name=agent_name, gateway_url=gateway_url, workdir=str(root)),
        encoding="utf-8",
    )
...
def local_send(
    ...
    _resolve_space = getattr(_gateway_cmd, "_resolve_space_via_cache")
    _resolve_session = getattr(_gateway_cmd, "_resolve_local_gateway_session", _resolve_local_gateway_session)
    _check_pending = getattr(_gateway_cmd, "_check_local_pending_replies", _check_local_pending_replies)
    _poll = getattr(_gateway_cmd, "_poll_local_inbox_over_http", _poll_local_inbox_over_http)
    _warn_pending = getattr(_gateway_cmd, "_print_pending_reply_warning_local", _print_pending_reply_warning_local)

    if space_id:
        resolved = _resolve_space(space_id)
        if resolved is None:
            raise typer.BadParameter(
        ...
        space_id = resolved
    try:
        resolved_session_token, connect_payload = _resolve_session(
            session_token=session_token,
            agent_name=agent_name,
            registry_ref=registry_ref,
            gateway_url=gateway_url,
