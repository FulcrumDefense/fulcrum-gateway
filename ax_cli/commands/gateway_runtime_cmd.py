"""ax gateway runtime — install + status + template catalog."""
...
from ..gateway_runtime_types import agent_template_list, runtime_type_list
...
def _agent_templates_payload() -> dict:
    ...
    _annotate = getattr(_gateway_cmd, "_annotate_template_taxonomy", _annotate_template_taxonomy)
    _ollama_status = getattr(_gateway_cmd, "ollama_setup_status", ollama_setup_status)
    _hermes_status = getattr(_gateway_cmd, "hermes_setup_status", hermes_setup_status)
    templates = [_annotate(item) for item in agent_template_list()]
    ...
    for item in templates:
        ...
        if template_id == "ollama":
            ...
            hermes_status = _hermes_status({"template_id": "hermes"})
...
def runtime_install(
    ...
    _install = getattr(_gateway_cmd, "_install_runtime_payload", _install_runtime_payload)
    _load_session = getattr(_gateway_cmd, "load_gateway_session", load_gateway_session)
    operator_session = _load_session()
    if not operator_session:
        err_console.print("[red]No active gateway session.[/red] Run `ax gateway login` first.")
        raise typer.Exit(1)
    try:
        payload = _install(template_id, target_override=target, operator_session=operator_session)
...
def runtime_types(as_json: bool = JSON_OPTION):
    ...
    _payload = getattr(_gateway_cmd, "_runtime_types_payload", _runtime_types_payload)
    payload = _payload()
    if as_json:
        print_json(payload)
        return
...
def templates(as_json: bool = JSON_OPTION):
    ...
    _payload = getattr(_gateway_cmd, "_agent_templates_payload", _agent_templates_payload)
    payload = _payload()
    if as_json:
        print_json(payload)
        return
