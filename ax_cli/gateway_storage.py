"""ax gateway storage — filesystem paths, registry/session/space-cache I/O, agent persistence."""
...
def daemon_status() -> dict[str, Any]:
    # Late-lookup so tests can monkeypatch these on ax_cli.gateway
    ...
    if pid_path().exists():
        try:
            ...
            pid = None
    running = _gw._pid_alive(pid)
    if not running:
        scanned = _gw._scan_gateway_process_pids()
        if scanned:
            pid = scanned[0]
            running = True
...
def _scan_process_pids(pattern: re.Pattern[str]) -> list[int]:
    # Late-lookup so tests can monkeypatch _pid_alive on ax_cli.gateway
    from . import gateway as _gw
    ...
    for raw_line in output.splitlines():
        ...
        try:
            ...
            continue
        if pid in {current_pid, parent_pid} or not _gw._pid_alive(pid):
            continue
...
def ui_status() -> dict[str, Any]:
    ...
    try:
        ...
        port = 8765
    running = _gw._pid_alive(pid_value)
    if not running:
        scanned = _gw._scan_gateway_ui_process_pids()
        if scanned:
            pid_value = scanned[0]
            running = True
    return {
...
def active_gateway_ui_pids() -> list[int]:
    ...
    if isinstance(pid, int) and status.get("running") and pid != os.getpid():
        pids.append(pid)
    pids.extend(_gw._scan_gateway_ui_process_pids())
    return sorted(set(pids))
...
def active_gateway_pids() -> list[int]:
    ...
    if isinstance(pid, int) and status.get("running") and pid != os.getpid():
        pids.append(pid)
    pids.extend(_gw._scan_gateway_process_pids())
    return sorted(set(pids))
