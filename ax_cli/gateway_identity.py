"""ax gateway identity — local session issue/verify, fingerprinting, identity binding, space authorization."""
...
def _runtime_origin_fingerprint(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    _hermes_sentinel_python = getattr(_gw, "_hermes_sentinel_python")
    _gateway_repo_root = getattr(_gw, "_gateway_repo_root")
    command = str(entry.get("exec_command") or "").strip() or None
