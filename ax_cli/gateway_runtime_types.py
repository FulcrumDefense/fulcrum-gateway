...
def agent_template_catalog() -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        "crewai": {
            "id": "crewai",
            "label": "CrewAI",
            "description": "CrewAI agent runtime managed by Gateway.",
            "availability": "ready",
            "launchable": True,
            "runtime_type": "exec",
            "asset_class": "interactive_agent",
            ...
            "return_paths": ["inline_reply"],
            "telemetry_shape": "basic",
            "suggested_name": "crewai-bot",
            "operator_summary": "Gateway-managed CrewAI bridge. Initial cut ships with a stub that exercises the API surface; real kickoff() execution with an LLM is a follow-up.",
            "recommended_test_message": "Reply with: CrewAI round trip OK.",
            "what_you_need": [
                "Python 3.11+ on this machine (the bridge runs as a Gateway-managed subprocess).",
                "For real Crew execution, install crewai in the bridge environment and configure an LLM (e.g. OPENAI_API_KEY). The stub bridge runs without either.",
            ],
            "setup_skill": "gateway-agent-setup",
            "setup_skill_path": str(skill_path),
            "defaults": {
                "runtime_type": "exec",
                "exec_command": "python3 examples/gateway_crewai/crewai_bridge.py",
                "workdir": str(repo_root),
            },
            "signals": runtime_signals["exec"],
            "advanced": {
                "adapter_label": "Gateway-managed CrewAI bridge",
                "supports_command_override": True,
            },
        },
        "hermes": {
