"""ax gateway daemon — GatewayDaemon (reconciliation loop, signal handling)."""
...
from .gateway_assets import hermes_setup_status, infer_operator_profile
...
from .gateway_health import _age_seconds, _now_iso, annotate_runtime_health
...
from .gateway_runtime import RuntimeLogger
from .gateway_state import _external_runtime_connected, _external_runtime_expected
from .gateway_storage import looks_like_space_uuid
...
class GatewayDaemon:
    ...
    def _reconcile_runtime(self, entry: dict[str, Any]) -> None:
        # Late-lookup so tests can monkeypatch ManagedAgentRuntime and
        # record_gateway_activity on ax_cli.gateway.
        ...
        if (external_runtime_state or _external_runtime_expected(entry)) and not _gw._is_hermes_plugin_runtime(
            runtime_type_lower
        ):
        ...
        if allowed_to_run:
            if runtime is not None:
                ...
                if changed_fields:
                    ...
                    runtime = None
            if runtime is None:
                runtime = _gw.ManagedAgentRuntime(entry, client_factory=self.client_factory, logger=self.logger)
    ...
    def _reconcile_registry(self, registry: dict[str, Any], session: dict[str, Any]) -> dict[str, Any]:
        ...
        for entry in agents:
            ...
            if current_attestation != previous_attestation:
                # Late-lookup so tests can monkeypatch record_gateway_activity.
                from . import gateway as _gw
    ...
    def run(self, *, once: bool = False) -> None:
        # Late-lookup so tests can monkeypatch session/registry/activity/pid
        # helpers on ax_cli.gateway.
        from . import gateway as _gw

        session = _gw.load_gateway_session()
        if not session:
            raise RuntimeError("Gateway login required. Run `ax gateway login` first.")

        existing_pids = _gw.active_gateway_pids()
        if existing_pids:
        ...
        _gw.write_gateway_pid(os.getpid())
        registry = _gw.load_gateway_registry()
        registry.setdefault("gateway", {})
        registry["gateway"]["last_started_at"] = registry["gateway"].get("last_started_at") or _now_iso()
        _gw.record_gateway_activity("gateway_started", pid=os.getpid())
        ...
        try:
            while not self._stop.is_set():
                ...
                _gw.save_gateway_registry(registry)
                if once:
                    break
            ...
            for runtime in runtimes:
                if _gw._is_hermes_sentinel_runtime(runtime.entry.get("runtime_type")):
                    runtime.stop(timeout=2.0)
            for runtime in runtimes:
                if not _gw._is_hermes_sentinel_runtime(runtime.entry.get("runtime_type")):
                    runtime.stop(timeout=1.0)
            final_registry = _gw.load_gateway_registry()
            ...
            _gw.save_gateway_registry(final_registry)
            _gw.record_gateway_activity("gateway_stopped")
            _gw.clear_gateway_pid(os.getpid())
