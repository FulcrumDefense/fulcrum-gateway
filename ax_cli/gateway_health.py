"""ax gateway health — runtime health annotation and scoring."""
...
from .gateway_assets import infer_operator_profile
...
from .gateway_identity import _parse_iso8601, evaluate_identity_space_binding
...
from .gateway_storage import load_gateway_registry
...
def annotate_runtime_health(
    ...
    *,
    ...
    # Late-lookup so tests can monkeypatch _pid_is_alive on ax_cli.gateway
    ...
    if profile["activation"] == "attach_only":
        local_pid_alive = str(enriched.get("desired_state") or "").lower() == "running" and _gw._pid_is_alive(
