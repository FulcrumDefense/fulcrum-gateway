"""Regression: CrewAI template registers correctly and the bridge
emits the Gateway lifecycle contract.

A Gateway-managed CrewAI runtime should be declarable from the
...
BRIDGE_PATH = REPO_ROOT / "examples" / "gateway_crewai" / "crewai_bridge.py"


def test_crewai_template_is_registered() -> None:
    catalog = agent_template_catalog()
    assert "crewai" in catalog, (
        "crewai template missing from agent_template_catalog. Should sit alongside langgraph / strands / ollama."
    )

    template = agent_template_definition("crewai")
    assert template["id"] == "crewai"
    assert template["runtime_type"] == "exec", (
        "Reuses the exec runtime adapter (same precedent as langgraph). "
        "A dedicated 'crewai' runtime_type is a follow-up."
    )
...
def test_crewai_template_default_exec_command_points_at_bridge() -> None:
    template = agent_template_definition("crewai")
    defaults = template.get("defaults") or {}
    exec_command = str(defaults.get("exec_command") or "")
    assert "examples/gateway_crewai/crewai_bridge.py" in exec_command, (
        f"crewai template's default exec_command should run the stub "
        f"bridge at examples/gateway_crewai/crewai_bridge.py. Got: {exec_command!r}"
    )


def test_crewai_template_listed_in_default_ordering() -> None:
    listed_ids = [item["id"] for item in agent_template_list()]
    assert "crewai" in listed_ids, (
        "crewai template should appear in the default (non-advanced) "
        "template list so the dashboard's Add Agent modal can render it."
    )


def test_crewai_bridge_file_exists() -> None:
    assert BRIDGE_PATH.exists(), (
        f"crewai bridge file missing at {BRIDGE_PATH}. The default "
        "exec_command in the template registration points at it."
    )


def test_crewai_bridge_emits_lifecycle_events(monkeypatch, capsys) -> None:
    ...
    try:
        import crewai_bridge as bridge
    finally:
        sys.path.pop(0)

    monkeypatch.setattr(sys, "argv", ["crewai_bridge.py", "test prompt"])
    monkeypatch.setattr(sys, "stdin", io.StringIO(""))
    monkeypatch.setenv("AX_GATEWAY_AGENT_NAME", "crewai-test")
